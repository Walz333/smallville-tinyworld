# Review Surface Drill Outcome Fields v1

## Purpose

This note defines the approved descriptive field vocabulary for review-surface drill outcomes. It is vocabulary only. It does not define schema requiredness, serialization, validation logic, data types, or file formats.

## Vocabulary Boundary

All fields in this layer remain subordinate to the frozen canonical fixture, diagnostic-output, review-surface, and drill layers.

All field meanings remain pre-decision only and may not convert a drill outcome into a review decision, approval state, execution state, workflow state, or control state.
...
Boundary note: must remain limited to `drill-summary`, `drill-pause-note`, `drill-stop-outcome`, or `drill-handoff-note`.
...
Meaning: identifies the already-presented frozen diagnostic output family from which the outcome preserves context.

Boundary note: may refer only to `summary`, `drift-note`, `stop-report`, `diagnostics-bundle`, or `export-summary`.

### `source_output_reference`

Meaning: records a reference to an already-presented diagnostic output only.

Boundary note: must refer only to an already-presented diagnostic output reference and may not function as replacement source authority.
...
Boundary note: canonical fixture files at canonical paths remain the source of truth wherever a drill outcome is recorded, read, or handed off.
...
Boundary note: remains a descriptive preservation indicator only and may not imply workflow state, queue state, action state, stop resolution, rerouting, or clearance.
...
Boundary note: remains a descriptive preservation indicator only and may not imply workflow state, queue state, action state, deferred work, or backlog semantics.

### `provenance_note`

Meaning: preserves provenance-aware review context tied to the underlying canonical fixture and source output.
...
Boundary note: packet content remains interpretive artifact only within drill outcomes and may not be restated as runtime world state.

### `water_mill_boundary_note`

Meaning: preserves Water Mill boundary context where relevant to the preserved outcome.

Boundary note: Water Mill remains design-asset-derived context only within drill outcomes and may not be restated as observed built authority without accepted evidence.
...
Boundary note: may support human handoff only and may not become execution handoff, routing state, or workflow transition.
...
Meaning: explicitly preserves that a drill outcome may not replace, extend, or restate a canonical review decision.

Boundary note: a drill outcome may reference a presented `stop-report` or a promoted review decision fixture as underlying subject matter, but may not replace, extend, or restate the canonical review decision.

### `runtime_state_forbidden`

Meaning: explicitly preserves that a drill outcome may not become runtime state or runtime-facing authority.

Boundary note: prohibition only, not a permitted state model.

### `workflow_state_forbidden`

Meaning: explicitly preserves that a drill outcome may not become workflow state or process-tracking state.

Boundary note: prohibition only, not a permitted workflow model.

### `execution_authority_forbidden`

Meaning: explicitly preserves that a drill outcome may not become execution authority or downstream clearance.
...
- introduces queue-like or workflow-like wording
